...
def print_info_to_slack(ticker1,ticker2):
    ...
    post_message(myToken,"#stock", "* Target({}) : ".format(ticker1) +str(target_price))

def sell_coin(ticker1, ticker2):
    bal_XRP = get_balance(ticker1)
    if bal_XRP * get_current_price(ticker2) > 5000:
        sell_result = upbit.sell_market_order(ticker2, bal_XRP * 0.9995)
...
while True:
    try:
        ...
        if start_time + datetime.timedelta(seconds=10) <= now < end_time - datetime.timedelta(seconds=10):
            ...
            if target_price_XRP < current_price_XRP and ma15 < current_price_XRP:
                krw = get_balance("KRW")
                if krw > 5000:
                    buy_result = upbit.buy_market_order("KRW-XRP", krw*0.9995)
                    post_message(myToken,"#stock", "XRP buy : " + str(buy_result))
        elif start_time < now  < start_time + datetime.timedelta(seconds=10): ## info print to slack
